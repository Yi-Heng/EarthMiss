...
class MetaRS(ERModule):
    ...
    def forward(self, x, y=None,ckpt=None):
        # if self.apply_mmr and ckpt.global_step == self.config.begin_mmr_iter:
        ...
        if self.training:
            ...
            if "mmr" in self.config.loss:
                if self.apply_mmr and ckpt.global_step > self.config.begin_mmr_iter:
                    loss_dict["mmr_loss"] = self.mmr_Loss(sar_w_list,rgb_w_list)
